...
class DashboardController extends Controller
{
    public function assoDashboard() {
        ...
        if(auth()->user()->isAsso) {
            $achat = null;
            $notView = Bingo::getNotViewResultInAsso();
            if(count($notView) > 0){
                $bingo = $notView[0];
                if($bingo != null && $bingo->id != null)
                    return redirect(route("mini-site-bingo-tirage", $bingo->id));
            }

            $dateActuelle = Carbon::now();

            $tombola = Tombola::getAllByIdAsso(auth()->user()->association->id);
            $bingo = Bingo::getAllByIdAsso(auth()->user()->association->id);

            $countOuvert =  count(Tombola::getAllEnCoursByIdAsso(auth()->user()->association_id));
            $countOuvert += count(Bingo::getAllEnCoursByIdAsso(auth()->user()->association_id));
            $don =  0;
