...
@section("body")
	<div id="__nuxt">
		<form method="post" action="{{ route('tombola-buy') }}">
			@csrf
			<div id="__layout">
				<div>
					...
					<main class="Register" style="--step-background-position: 0%;">
						...
						<div class="Logo Register--Logo"  style="margin-bottom:0px"><img src="{{asset('assets/img/logo/'. $tombola->association->id . '/'. $tombola->association->logo)}}"
							...
							<section class="RegisterOrganism Register--Step" id="section1" style="margin-top:0px">
								<fieldset>
									...
									<div class="row g-3 align-items-center">
										...
										<div class="HaFormField--Label">
											<label for="don-tombola-buy" class="Label">
							...
							<section class="RegisterOrganism Register--Step" id="section2" style="margin-top:0px;display:none">
								<fieldset>
									<legend>Don pour {{ $tombola->association->name }}</legend>
									...
									<input type="hidden" id="don-tombola-buy" required value="{{$tombola->id}}"
														name="tombolaId" class="Input Input--Limit">
								</fieldset>
...
@section("footer")
	<script>
		...
		$("#btn-suivant-2").off().on("click", function(){
			if($("#name-tombola-buy").val() != "" && $("#email-tombola-buy").val() != "" ) {
				$("#section2").hide(1000);
		...
		function changeValueDon() {
			...
			if(!<?= $tombola->useTirage ?> && total > <?= $tombola->ticket ?>)
				$("#erreur-don").html("Nous somme désolé mais il reste que " +  <?= $tombola->ticket ?>+ " ticket(s) alors qu'avec ce don + achat vous obtenez au total " + total + " tickets");
			else 
				$("#erreur-don").html("");
		}

		function changeBuyPlus() {
			const useTirage = <?= $tombola->useTirage ?>;
			if(!useTirage) {
				var erreur = ""
				var ticket = $("#plus-ticket-buy").val();
				max =  Math.round((<?= $tombola->max ?> * 30 ) / 100); 
				if(ticket > max) {
				...
				if(ticket > <?= $tombola->ticket ?>) {
					erreur = "Nous somme désolé mais il reste que " +  <?= $tombola->ticket ?>+ " ticket(s)";
					ticket = <?= $tombola->ticket ?>;
					$("#plus-ticket-buy").val(<?= $tombola->ticket ?>);
				}
		...
		$(".Input--Limit").off().on("keyup", function() {
			...
			if($(this) != null && typeof $(this).data("max") != "undefined") {
				...
				if($(this).attr("id") == "don-tombola-buy") {
					changeValueDon();
				}
			}
		});
