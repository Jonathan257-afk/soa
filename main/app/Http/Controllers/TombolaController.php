...
class TombolaController extends Controller
{
    ...
    public function site($id) {
        $res = toApiGetway("tombola", "getAllByIdAsso",[
            "id" => auth()->user()->association_id,
        ]);
        $tombola = ($res != null && $res["res"] && $res["data"]) ? $res["data"] : null;
        if($tombola == null)
    ...
    public function index() {
        if(auth()->user()->isAsso) {
            ...
            $tombolas = ($resTombola != null && $resTombola["res"] && $resTombola["data"]) ? $resTombola["data"] : [];
                ...
                if($request->has("numberLotAdd")) {
                    ...
                    for($i = 0; $i < $nombreLot ; $i++) { 
                        if($request->has("libelleLot". $i) && $request->has("numberLot". $i)  && $request->has("nameImageLot". $i) ) {
                            $params["libelleLot". $i] = $request->input("libelleLot". $i);
                            $params["numberLot". $i] = $request->input("numberLot". $i);
                            $params["nameImageLot". $i] = $request->input("nameImageLot". $i);
                            $params["linkLot". $i] = $request->input("linkLot". $i);
                            $params["price". $i] = $request->input("price". $i);
                        }
                        
                    }
                }
                $tombola = toApiGetway("tombola", "store",$params);

                if($tombola != null && $tombola["res"] && $tombola["data"]) {      
                    if($request->has("numberLotAdd")) {
                        $nombreLot = intval($request->input("numberLotAdd"));
                        for($i = 0; $i < $nombreLot ; $i++) { 
                            if($request->has("libelleLot". $i) && $request->has("numberLot". $i)  && $request->has("nameImageLot". $i) ) {
                                ...
                                if(File::exists(public_path()."/assets/file/tmp/".$request->input("nameImageLot". $i))) {
                                    $destinationPath = public_path()."/assets/img/lot/".$tombola["data"]["id"]."/".$request->input("nameImageLot". $i);
