...
class BingoController extends Controller
{
    ...
    public function index() {
        if(auth()->user()->isAsso) {
            ...
            $bingos = ($resBingo != null && $resBingo["res"] && $resBingo["data"]) ? $resBingo["data"] : [];
    ...
    public function store(Request $request) {
        if(auth()->user()->isAsso) {
            ...
            if($dateDebut != "" && $date != "" && $libelle != ""  && $montant > 0  && $ticket  > 0 ) {
                ...
                $bingo = toApiGetway("bingo", "store",$params);

                if($bingo != null && $bingo["res"] && $bingo["data"]) {      
                    if($request->has("numberLotAdd")) {
                        $nombreLot = intval($request->input("numberLotAdd"));
                        for($i = 0; $i < $nombreLot ; $i++) { 
                            if($request->has("libelleLot". $i) && $request->has("numberLot". $i)  && $request->has("nameImageLot". $i) ) {
                                if(File::exists(public_path()."/assets/file/tmp/".$request->input("nameImageLot". $i))) {
                                    $destinationPath = public_path()."/assets/img/lot/bingo/".$bingo['data']['id']."/".$request->input("nameImageLot". $i);
