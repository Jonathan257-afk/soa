...
class AssociationController extends Controller
{
    ...
    public function signUp(Request $request) {
        ...
        if($request->input("password") == $request->input("confirm")) {
            ...
            $association = toApiGetway("association", "store",[
                "name" => $request->input("name"), 
                "email" => $request->input("email"), 
                "logo_name" => $logo_name, 
                "interet" => $request->input("interet"), 
                "cerfa"=> $request->input("cerfa")
            ]);
          
            if($association['res'] && $association['data']) {
                if($request->has("logo"))
                    $request->file('logo')->move(public_path()."/assets/img/logo/". $association['data']['id'], $request->input('logo_name'));
                else  {
                    if(File::exists(public_path()."/assets/img/logoGreen.png")) {
                        $destinationPath = public_path()."/assets/img/logo/". $association['data']['id'] ."/"."logoGreen.png";
                ...
                if(User::signUpAssociation($request->input("name"), $request->input("email"), $request->input("password"), $association['data']['id'])) {
                    return view("page.auth.confirmaccount")->with("success", "Votre compte a été crée avec success !");
                }
            }
